...
时间线 SHALL 按 watchedAt 月份分组、组内按观看日期倒序（最新在前）纵向排列；每条目 SHALL 展示：海报缩略位（与海报墙一致的色卡解析）、标题、剧集徽标（mediaType 为 episode 时显示 SxxExx，电影不显示徽标）、观看日期、评分摘要（rating 为 0 显示「未评分」）。分组与排序 SHALL 为纯函数 selector，在纯 Node 环境可测。
