...
ui-contracts SHALL 导出最小设计令牌集（颜色、间距、圆角、字重、海报占位色板 ≥8 色、热力图 5 级强度色），以普通 JS 对象供移动端与桌面端共同消费。海报占位色板 SHALL 提供确定性映射函数（输入 record id，输出唯一色槽），相同 id 在双端 MUST 得到相同颜色。

#### Scenario: 占位色确定性
- **WHEN** 对同一 record id 在两端分别调用占位色映射
- **THEN** 返回相同色板槽位（颜色值相等）
