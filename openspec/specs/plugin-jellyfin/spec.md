# plugin-jellyfin Specification

## Purpose

Jellyfin 观影历史同步插件：凭证配置（加密落盘）、headless 客户端口（svc:http）、升序分批增量同步（断点游标 + 身份去重 + 逐页进度事件）、item → 记录快照映射；写入经 edit 唯一通道。
...
同步 SHALL 采用**分批渐进**策略（大库容错）：按 `DatePlayed` 升序（从远到近）逐页拉取；新条目攒满批次阈值（默认 1000 条）即提交落盘一批；**每页之间停顿（默认 300ms）以规避限速断连**；**每页拉取后**发布 `jellyfin:sync-progress` 事件（`{page, fetched, imported}`，页码逐页递增，纯翻页无新增时同样发布，UI 进度逐步可见）；同步游标（skip 偏移，绑定 baseUrl）SHALL 持久化于 syncMeta 并按页推进。中途失败 SHALL 保留已提交批次与游标，重试从断点续传；游标到达列表尾部后，后续同步仅从尾部拉取新增。游标超出列表尾部（skip > total 且空页，如服务器删除历史或旧版 bug 遗留）时 SHALL 自动归零从头重扫一次（身份去重保证不重复导入）；Skip/Take 时代写入的 v1 游标 SHALL 在加载时一次性丢弃。
...
#### Scenario: 不可信游标自愈
- **WHEN** 游标超出服务器总数（旧 bug 遗留或历史删除）或游标为 v1 格式
- **THEN** 游标归零重扫，缺失条目补齐、已有条目不重复导入
